...
import 'package:learning_app/core/diff_match/common.dart';
import 'package:learning_app/core/diff_match/diff/cleanup.dart';
import 'package:learning_app/core/diff_match/diff/diff.dart';
import 'package:learning_app/core/diff_match/diff/main.dart';
import 'package:learning_app/core/diff_match/diff/utils.dart';
import 'package:learning_app/core/diff_match/match.dart';
  ...
  while (textPointer < text.length) {
    ...
    while (textPointer < text.length) {
      if (text[textPointer].isNotEmpty) {
        ...
        var line = '';
        try {
          line = Uri.decodeFull(text[textPointer].characters.safeSubstring(1));
        } on ArgumentError {
...
void patchAddContext(Patch patch, String text, int patchMargin) {
  ...
  var pattern =
      text.characters.safeSubstring(patch.start2, patch.start2 + patch.length1);
  var padding = 0;
  ...
  while ((text.indexOf(pattern) != text.lastIndexOf(pattern)) &&
    ...
    padding += patchMargin;
    pattern = text.characters.safeSubstring(max(0, patch.start2 - padding),
  ...
  final prefix = text.characters
      .safeSubstring(max(0, patch.start2 - padding), patch.start2);
  if (prefix.isNotEmpty) {
    patch.diffs.insert(0, Diff(DIFF_EQUAL, prefix));
  }
  // Add the suffix.
  final suffix = text.characters.safeSubstring(patch.start2 + patch.length1,
      min(text.length, patch.start2 + patch.length1 + padding));
  ...
  for (var aDiff in diffs) {
    ...
    switch (aDiff.operation) {
      case DIFF_INSERT:
        ...
        postpatch_buffer
          ..write(postpatch_text.characters.safeSubstring(0, char_count2))
          ..write(aDiff.text)
          ..write(postpatch_text.characters.safeSubstring(char_count2));
      ...
      case DIFF_DELETE:
        ...
        postpatch_buffer
          ..write(postpatch_text.characters.safeSubstring(0, char_count2))
          ..write(postpatch_text.characters
              .safeSubstring(char_count2 + aDiff.text.length));
        postpatch_text = postpatch_buffer.toString();
        break;
      case DIFF_EQUAL:
...
List patchApply(List<Patch> patches, String text,
  ...
  for (var aPatch in patches) {
    ...
    if (text1.length > BITS_PER_INT) {
      ...
      // a monster delete.
      start_loc = match(
          text, text1.characters.safeSubstring(0, BITS_PER_INT), expected_loc,
          threshold: matchThreshold, distance: matchDistance);
      if (start_loc != -1) {
        end_loc = match(
            text,
            text1.characters.safeSubstring(text1.length - BITS_PER_INT),
    ...
    if (start_loc == -1) {
      ...
      String text2;
      if (end_loc == -1) {
        text2 = text.characters.safeSubstring(
            start_loc, min(start_loc + text1.length, text.length));
      } else {
        text2 = text.characters
            .safeSubstring(start_loc, min(end_loc + BITS_PER_INT, text.length));
      }
      if (text1 == text2) {
        // Perfect match, just shove the replacement text in.
        text_buffer.clear();
        text_buffer
          ..write(text.characters.safeSubstring(0, start_loc))
          ..write(diffText2(aPatch.diffs))
          ..write(text.characters.safeSubstring(start_loc + text1.length));
          ...
          for (var aDiff in aPatch.diffs) {
            if (aDiff.operation != DIFF_EQUAL) {
              ...
              if (aDiff.operation == DIFF_INSERT) {
                ...
                text_buffer.clear();
                text_buffer
                  ..write(text.characters.safeSubstring(0, start_loc + index2))
                  ..write(aDiff.text)
                  ..write(text.characters.safeSubstring(start_loc + index2));
                text = text_buffer.toString();
                ...
                text_buffer
                  ..write(text.characters.safeSubstring(0, start_loc + index2))
                  ..write(text.characters.safeSubstring(start_loc +
  ...
  text = text.characters
      .safeSubstring(nullPadding.length, text.length - nullPadding.length);
  return [text, results];
}
...
String patchAddPadding(List<Patch> patches, {int margin = 4}) {
  ...
  if (diffs.isEmpty || diffs[0].operation != DIFF_EQUAL) {
    ...
    firstDiff.text =
        '${nullPadding.characters.safeSubstring(firstDiff.text.length)}${firstDiff.text}';
  ...
  if (diffs.isEmpty || diffs.last.operation != DIFF_EQUAL) {
    ...
    lastDiff.text =
        '${lastDiff.text}${nullPadding.characters.safeSubstring(0, extraLength)}';
    patch.length1 += extraLength;
    patch.length2 += extraLength;
  }
    ...
    while (bigpatch.diffs.isNotEmpty) {
      ...
      while (bigpatch.diffs.isNotEmpty && patch.length1 < patch_size - margin) {
        ...
        if (diff_type == DIFF_INSERT) {
          ...
          diff_text = diff_text.characters.safeSubstring(
          ...
          if (diff_text == bigpatch.diffs[0].text) {
            ...
            bigpatch.diffs[0].text = bigpatch.diffs[0].text.characters
                .safeSubstring(diff_text.length);
          }
        }
      }
      // Compute the head context for the next patch.
      precontext = diffText2(patch.diffs);
      precontext = precontext.characters
          .safeSubstring(max(0, precontext.length - margin));
      // Append the end context for this patch.
      String postcontext;
      if (diffText1(bigpatch.diffs).length > margin) {
        postcontext =
            diffText1(bigpatch.diffs).characters.safeSubstring(0, margin);
      } else {
