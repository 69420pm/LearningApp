...
String _linesToCharsMunge(
  ...
  // Walk the text, pulling out a.characters.safeSubstring for each line.
  ...
  while (lineEnd < text.length - 1) {
    ...
    line = text.characters.safeSubstring(lineStart, lineEnd + 1);
    lineStart = lineEnd + 1;
...
int commonPrefix(String text1, String text2) {
  ...
  final n = min(text1.characters.length, text2.characters.length);
  for (var i = 0; i < n; i++) {
    if (text1.characters.characterAt(i) != text2.characters.characterAt(i)) {
      return i;
    }
  }
  return n;
...
int commonSuffix(String text1, String text2) {
  ...
  for (var i = 1; i <= n; i++) {
    if (text1.characters.characterAt(text1_length - i) !=
        text2.characters.characterAt(text2_length - i)) {
      return i - 1;
    }
  }
  return n;
...
int commonOverlap(String text1, String text2) {
  ...
  if (text1_length > text2_length) {
    text1 = text1.characters.safeSubstring(text1_length - text2_length);
  } else if (text1_length < text2_length) {
    text2 = text2.characters.safeSubstring(0, text1_length);
  }
  ...
  var length = 1;
  while (true) {
    var pattern = text1.characters.safeSubstring(text_length - length);
    ...
    length += found;
    if (found == 0 ||
        text1.characters.safeSubstring(text_length - length) ==
            text2.characters.safeSubstring(0, length)) {
      best = length;
      length++;
    }
  }
