...
/// Do the two texts share a.characters.safeSubstring which is at least half the length of
/// the longer text?
///
...
/// Does a.characters.safeSubstring of [shorttext] exist within [longtext] such that the
///.characters.safeSubstring is at least half the length of [longtext]?
///
/// * [longtext] is the longer string.
/// * [shorttext is the shorter string.
/// * [i] Start index of quarter length.characters.safeSubstring within longtext.
///
...
List<String>? _diffHalfMatchI(String longtext, String shorttext, int i) {
  // Start with a 1/4 length.characters.safeSubstring at position i as a seed.
  final seed = longtext.characters
      .safeSubstring(i, i + (longtext.length / 4).floor().toInt());
  var j = -1;
  var best_common = '';
  var best_longtext_a = '', best_longtext_b = '';
  var best_shorttext_a = '', best_shorttext_b = '';
  while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
    var prefixLength = commonPrefix(longtext.characters.safeSubstring(i),
        shorttext.characters.safeSubstring(j));
    var suffixLength = commonSuffix(longtext.characters.safeSubstring(0, i),
        shorttext.characters.safeSubstring(0, j));
    if (best_common.length < suffixLength + prefixLength) {
      best_common = '${shorttext.characters.safeSubstring(j - suffixLength, j)}'
          '${shorttext.characters.safeSubstring(j, j + prefixLength)}';
      best_longtext_a = longtext.characters.safeSubstring(0, i - suffixLength);
      best_longtext_b = longtext.characters.safeSubstring(i + prefixLength);
      best_shorttext_a =
          shorttext.characters.safeSubstring(0, j - suffixLength);
      best_shorttext_b = shorttext.characters.safeSubstring(j + prefixLength);
    }
  }
