  ...
  while (pointer < diffs.length) {
    if (diffs[pointer].operation == DIFF_EQUAL) {
      ...
      if (lastequality != null &&
        ...
        // Duplicate record.
        diffs.insert(equalities.last, Diff(DIFF_DELETE, lastequality));
      ...
      if (overlap_length1 >= overlap_length2) {
        if (overlap_length1 >= deletion.length / 2 ||
          ...
          diffs.insert(
              pointer,
              Diff(DIFF_EQUAL,
                  insertion.characters.safeSubstring(0, overlap_length1)));
          diffs[pointer - 1].text = deletion.characters
              .safeSubstring(0, deletion.length - overlap_length1);
          diffs[pointer + 1].text =
              insertion.characters.safeSubstring(overlap_length1);
          pointer++;
        }
      } else {
        if (overlap_length2 >= deletion.length / 2 ||
          ...
          diffs.insert(
              pointer,
              Diff(DIFF_EQUAL,
                  deletion.characters.safeSubstring(0, overlap_length2)));
          diffs[pointer - 1] = Diff(
              DIFF_INSERT,
              insertion.characters
                  .safeSubstring(0, insertion.length - overlap_length2));
          diffs[pointer + 1] = Diff(
              DIFF_DELETE, deletion.characters.safeSubstring(overlap_length2));
          pointer++;
        }
      }
      pointer++;
...
void cleanupSemanticLossless(List<Diff> diffs) {
  ...
  while (pointer < diffs.length - 1) {
    if (diffs[pointer - 1].operation == DIFF_EQUAL &&
      ...
      if (commonOffset != 0) {
        var commonString =
            edit.characters.safeSubstring(edit.length - commonOffset);
        equality1 = equality1.characters
            .safeSubstring(0, equality1.length - commonOffset);
        edit =
            '$commonString${edit.characters.safeSubstring(0, edit.length - commonOffset)}';
      ...
      while (
        ...
        edit = '${edit.characters.safeSubstring(1)}${equality2[0]}';
        equality2 = equality2.characters.safeSubstring(1);
...
void cleanupMerge(List<Diff> diffs) {
  diffs.add(Diff(DIFF_EQUAL, '')); // Add a dummy entry at the end.
  var pointer = 0;
  var count_delete = 0;
  var count_insert = 0;
  var text_delete = '';
            ...
            if (commonlength != 0) {
              if ((pointer - count_delete - count_insert) > 0 &&
                ...
                diffs[i].text = '${diffs[i].text}'
                    '${text_insert.characters.safeSubstring(0, commonlength)}';
              } else {
                diffs.insert(
                    0,
                    Diff(DIFF_EQUAL,
              ...
              text_insert = text_insert.characters.safeSubstring(commonlength);
              text_delete = text_delete.characters.safeSubstring(commonlength);
            }
            // Factor out any common suffixies.
            commonlength = commonSuffix(text_insert, text_delete);
            if (commonlength != 0) {
              diffs[pointer].text =
                  '${text_insert.characters.safeSubstring(text_insert.length - commonlength)}${diffs[pointer].text}';
              text_insert = text_insert.characters
                  .safeSubstring(0, text_insert.length - commonlength);
              text_delete = text_delete.characters
                  .safeSubstring(0, text_delete.length - commonlength);
            }
          }
  ...
  while (pointer < diffs.length - 1) {
    if (diffs[pointer - 1].operation == DIFF_EQUAL &&
      ...
      if (diffs[pointer]
          .text
          .characters
          .endsWith(diffs[pointer - 1].text.characters)) {
        // Shift the edit over the previous equality.
        diffs[pointer].text = '${diffs[pointer - 1].text}'
            '${diffs[pointer].text.characters.safeSubstring(0, diffs[pointer].text.characters.length - diffs[pointer - 1].text.characters.length)}';
        diffs[pointer + 1].text =
        ...
        changes = true;
          ...
          .text
          .characters
          .startsWith(diffs[pointer + 1].text.characters)) {
        // Shift the edit over the next equality.
        diffs[pointer - 1].text =
            '${diffs[pointer - 1].text}${diffs[pointer + 1].text}';
        diffs[pointer].text =
            '${diffs[pointer].text.characters.safeSubstring(diffs[pointer + 1].text.length)}'
            '${diffs[pointer + 1].text}';
        diffs.removeRange(pointer + 1, pointer + 2);
        changes = true;
      }
