...
import 'package:learning_app/core/diff_match/diff/half_match.dart';
...
List<Diff> diff(String text1, String text2,
  ...
  var commonprefix = text1.characters.safeSubstring(0, commonlength);
  text1 = text1.characters.safeSubstring(commonlength);
  text2 = text2.characters.safeSubstring(commonlength);

  // Trim off common suffix (speedup).
  commonlength = commonSuffix(text1, text2);
  var commonsuffix =
      text1.characters.safeSubstring(text1.characters.length - commonlength);
  text1 =
      text1.characters.safeSubstring(0, text1.characters.length - commonlength);
  text2 =
      text2.characters.safeSubstring(0, text2.characters.length - commonlength);
...
List<Diff> _diffCompute(String text1, String text2, double timeout,
  ...
  var longtext =
      text1.characters.length > text2.characters.length ? text1 : text2;
  var shorttext =
      text1.characters.length > text2.characters.length ? text2 : text1;
  var i = findCharacterIndex(longtext.characters, shorttext.characters);

  if (i != -1) {
    // Shorter text is inside the longer text (speedup).
    var op = (text1.characters.length > text2.characters.length)
        ? DIFF_DELETE
        : DIFF_INSERT;
    diffs.add(Diff(op, longtext.characters.safeSubstring(0, i)));
    diffs.add(Diff(DIFF_EQUAL, shorttext));
    diffs.add(Diff(op,
        longtext.characters.safeSubstring(i + shorttext.characters.length)));
    return diffs;
  }

  if (shorttext.characters.length == 1) {
  ...
  if (checklines &&
      text1.characters.length > 100 &&
      text2.characters.length > 100) {
...
List<Diff> diffBisect(
  ...
  final text1_length = text1.characters.length;
  final text2_length = text2.characters.length;
...
List<Diff> _diffBisectSplit(String text1, String text2, int x, int y,
  ...
  final text1a = text1.characters.safeSubstring(0, x);
  final text2a = text2.characters.safeSubstring(0, y);
  final text1b = text1.characters.safeSubstring(x);
  final text2b = text2.characters.safeSubstring(y);
...
int findCharacterIndex(Characters longChars, Characters shortChars) {
  int index = 0;

  for (var it = longChars.iterator; it.moveNext(); index++) {
    // Extract a substring of the same length as shortChars for comparison
    Characters substring = longChars.skip(index).take(shortChars.length);
    if (substring.string == shortChars.string) {
      return index;
    }
  }
